/**
 * XinWiki embed widget SDK — floating chat launcher.
 *
 * Programmatic:
 *   XinWiki.init({ channel, token, position, primaryColor, title, baseUrl })
 *   XinWiki.open() | close() | toggle() | destroy()
 *   XinWiki.on('ready', fn) | off('ready', fn)
 *
 * Host context & actions (postMessage to iframe):
 *   XinWiki.setContext({ userId, page, ... })
 *     Inject visitor/page context merged into each chat query.
 *   XinWiki.openWithQuery('How do I reset my password?')
 *     Opens the panel (if closed) and sends the query when the iframe is ready.
 *   XinWiki.setLocale('en-US')
...
(function (global) {
  ...
  function emit(event, payload) {
    ...
    handlers.slice().forEach(function (fn) {
      try { fn(payload); } catch (e) { console.error('[XinWiki]', e); }
    });
  }
  ...
  function createWidget(opts) {
    ...
    if (!channelId || (!staticToken && !tokenEndpoint)) {
      console.warn('[XinWiki] channel and (token or tokenEndpoint) are required');
      return null;
    }
    ...
    function loadToken(force) {
      ...
      tokenInFlight = fetch(tokenEndpoint, {
        ...
        .catch(function (e) {
          console.error('[XinWiki] failed to load token', e);
          throw e;
        })
    ...
    if (!baseUrl) {
      ...
      if (script && script.src) {
        baseUrl = script.src.replace(/\/xinwiki-widget\.js.*$/, '');
      } else {
    ...
    function postHostPayload(type, payload) {
      if (!iframe.contentWindow) {
        console.warn('[XinWiki] iframe not ready');
        return false;
      }
    ...
    function setContext(ctx) {
      if (!ctx || typeof ctx !== 'object') {
        console.warn('[XinWiki] setContext expects an object');
        return;
      }
    ...
    function openWithQuery(query) {
      ...
      if (!text) {
        console.warn('[XinWiki] openWithQuery requires a non-empty query');
        return;
      }
      setOpen(true);
    ...
    function setLocale(locale) {
      ...
      if (!loc) {
        console.warn('[XinWiki] setLocale requires a locale string');
        return;
      }
