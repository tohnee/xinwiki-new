...
export const XINWIKI_CLOUD_BASE_URL = 'https://xinwiki.weixin.qq.com'
export const XINWIKI_CLOUD_PROVIDER = 'xinwikicloud'
...
export function isXinWikiCloudModel(model: ModelConfig): boolean {
  return model.parameters?.provider === XINWIKI_CLOUD_PROVIDER
}

/** Returns kinds that already have a XinWikiCloud model of the matching backend type. */
export function existingWkcKinds(models: ModelConfig[]): Set<WkcModelKind> {
  const found = new Set<WkcModelKind>()
  for (const model of models) {
    if (!isXinWikiCloudModel(model)) continue
...
export function buildWkcModelConfig(
  ...
  const parameters: ModelConfig['parameters'] = {
    base_url: XINWIKI_CLOUD_BASE_URL,
