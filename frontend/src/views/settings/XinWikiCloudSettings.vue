<template>
  <div class="xinwikicloud-settings">
    <div class="section-header">
      <h2>{{ $t('settings.xinwikiCloud.title') }}</h2>
      <p class="section-description">
        {{ $t('settings.xinwikiCloud.description') }}
        <a
          class="doc-link"
        ...
        >
          {{ $t('settings.xinwikiCloud.viewDocs') }}
    ...
    <div v-if="credentialState === 'unconfigured'" class="credential-status unconfigured">
      ...
      <span>{{ $t('settings.xinwikiCloud.unconfigured') }}</span>
    </div>
    ...
    <div v-else-if="credentialState === 'expired'" class="credential-warning">
      ...
      <div class="warning-text">
        <strong>{{ $t('settings.xinwikiCloud.expired') }}</strong><br />
        {{ reinitReason || $t('settings.xinwikiCloud.expiredDefault') }}
      </div>
    </div>
    ...
    <div v-else-if="credentialState === 'configured'" class="credential-status success">
      ...
      <span class="status-text">{{ $t('settings.xinwikiCloud.configured') }}</span>
      <t-button
        v-if="!formExpanded"
        variant="outline"
        theme="default"
      ...
      >
        ...
        {{ $t('settings.xinwikiCloud.reconfigure') }}
      </t-button>
    </div>
    ...
    <div v-if="formExpanded" class="settings-group">
      <div class="setting-row">
        <div class="setting-info">
          <label class="setting-label">{{ $t('settings.xinwikiCloud.appIdLabel') }}</label>
          <p class="setting-desc">{{ $t('settings.xinwikiCloud.appIdDesc') }}</p>
        </div>
        <div class="setting-control">
          <t-input
            v-model="form.appId"
            :placeholder="$t('settings.xinwikiCloud.appIdPlaceholder')"
            autocomplete="off"
            style="width: 280px;"
          />
        </div>
      ...
      <div class="setting-row">
        <div class="setting-info">
          <label class="setting-label">{{ $t('settings.xinwikiCloud.appSecretLabel') }}</label>
          <p class="setting-desc">{{ $t('settings.xinwikiCloud.appSecretDesc') }}</p>
        </div>
        <div class="setting-control">
          <t-input
            v-model="form.appSecret"
            type="password"
            :placeholder="$t('settings.xinwikiCloud.appSecretPlaceholder')"
      ...
      <div class="setting-row action-row">
        <div class="setting-info">
          <p class="setting-desc">{{ $t('settings.xinwikiCloud.saveHint') }}</p>
        </div>
        <div class="setting-control">
          <t-button
            theme="primary"
            ...
            @click="handleSave"
          >
            {{ $t('settings.xinwikiCloud.saveBtn') }}
          </t-button>
        </div>
      </div>
    </div>
    ...
    >
      <div class="models-section__header">
        <h3 class="models-section__title">{{ $t('settings.xinwikiCloud.modelsSection.title') }}</h3>
        <p class="models-section__desc">
          {{
            credentialState === 'configured'
              ? $t('settings.xinwikiCloud.modelsSection.descReady')
              : $t('settings.xinwikiCloud.modelsSection.descPending')
          }}
        </p>
      </div>
        ...
        >
          ...
          <div class="model-row__action">
            <t-tag
              ...
              size="small"
            >
              {{ $t('settings.xinwikiCloud.modelsSection.statusAdded') }}
            </t-tag>
            <t-popconfirm
              v-else-if="credentialState === 'configured'"
              :content="$t('settings.xinwikiCloud.modelsSection.confirmAddOne', {
                type: kindLabel(kind),
                name: WKC_MODEL_NAME_BY_KIND[kind],
              })"
              :confirm-btn="{ content: $t('settings.xinwikiCloud.modelsSection.addOne'), theme: 'primary' }"
            ...
            >
              <t-button
                ...
                @click.stop
              >
                {{ $t('settings.xinwikiCloud.modelsSection.addOne') }}
              </t-button>
            </t-popconfirm>
            <span v-else class="model-row__pending">
              {{ $t('settings.xinwikiCloud.modelsSection.statusPending') }}
            </span>
          </div>
        </div>
      </div>
      ...
      >
        <t-popconfirm
          :content="$t('settings.xinwikiCloud.modelsSection.confirmAddAll', { count: missingKinds.length })"
          :confirm-btn="{
            content: $t('settings.xinwikiCloud.modelsSection.addAllConfirm'),
            theme: 'primary',
          }"
          :cancel-btn="{ content: $t('common.cancel') }"
          placement="top"
        ...
        >
          ...
          >
            {{ $t('settings.xinwikiCloud.modelsSection.addAllBtn', { count: missingKinds.length }) }}
          </t-button>
        </t-popconfirm>
      </div>
      ...
      >
        ...
        {{ $t('settings.xinwikiCloud.modelsSection.allReady') }}
      </p>
    </section>

    <!-- 使用说明 -->
    <div class="usage-hint">
      <p class="hint-title">{{ $t('settings.xinwikiCloud.usageTitle') }}</p>
      <p class="hint-text" v-html="$t('settings.xinwikiCloud.usageSteps').replace(/\n/g, '<br />')" />
    </div>
  </div>
</template>
...
import { saveXinWikiCloudCredentials, getXinWikiCloudStatus, createModel, listModels } from '@/api/model'
import { testEmbeddingModel } from '@/api/initialization'
import {
  WKC_MODEL_KINDS,
  WKC_MODEL_NAME_BY_KIND,
  XINWIKI_CLOUD_BASE_URL,
  XINWIKI_CLOUD_PROVIDER,
  buildWkcModelConfig,
  existingWkcKinds,
  type WkcModelKind,
} from '@/utils/xinwikiCloudModels'
...
const kindDisplayName = (kind: WkcModelKind) =>
  t(`settings.xinwikiCloud.addModelsDisplayName.${kind}`)
...
const resolveEmbeddingDimension = async (): Promise<number> => {
  const result = await testEmbeddingModel({
    ...
    baseUrl: XINWIKI_CLOUD_BASE_URL,
    provider: XINWIKI_CLOUD_PROVIDER,
  })
  if (!result.available || !result.dimension) {
    throw new Error(result.message || t('settings.xinwikiCloud.addModelsEmbeddingFailed'))
  }
  ...
  try {
    for (const kind of targets) {
      try {
        ...
        console.error(`Failed to create XinWikiCloud ${kind} model:`, err)
        failed += 1
      }
    }

    if (success > 0 && failed === 0) {
      MessagePlugin.success(t('settings.xinwikiCloud.addModelsSuccess', { count: success }))
    } else if (success > 0) {
      MessagePlugin.warning(t('settings.xinwikiCloud.addModelsPartial', { success, failed }))
    } else {
      MessagePlugin.error(t('settings.xinwikiCloud.addModelsFailed'))
    }
  } finally {
...
const handleSave = async () => {
  if (!form.value.appId || !form.value.appSecret) {
    MessagePlugin.warning(t('settings.xinwikiCloud.fillRequired'))
    return
  }
  saving.value = true
  try {
    await saveXinWikiCloudCredentials({
      app_id: form.value.appId,
      app_secret: form.value.appSecret,
    })
    MessagePlugin.success(t('settings.xinwikiCloud.saveSuccess'))
    form.value.appId = ''
    ...
    MessagePlugin.error(err?.message || t('settings.xinwikiCloud.saveFailed'))
  } finally {
    saving.value = false
  }
}
...
const checkStatus = async () => {
  try {
    const status = await getXinWikiCloudStatus()
