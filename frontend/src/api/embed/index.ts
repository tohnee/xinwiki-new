...
export const EMBED_CHAT_SESSION_STORAGE_PREFIX = 'xinwiki-embed-session:'
...
const EMBED_MSG_SOURCE = 'xinwiki-embed'
const EMBED_HOST_SOURCE = 'xinwiki-host'
...
export function buildWidgetSnippet(
  ...
  const attrs = [
    `src="${escapeHtmlAttr(`${base}/xinwiki-widget.js`)}"`,
...
export const SECURE_TOKEN_ENDPOINT_PLACEHOLDER = 'https://your-backend.example.com/xinwiki/embed-token'
...
export function buildSecureServerNodeExample(channelId: string, opts?: { baseUrl?: string }) {
  ...
  return [
    `// Node/Express — keep XINWIKI_PUBLISH_TOKEN only on the server (env var).`,
...
export function buildSecureServerGoExample(channelId: string, opts?: { baseUrl?: string }) {
  ...
  const exchangeUrl = `${base}/api/v1/embed/${channelId}/exchange`
  return [
    `// Go net/http — keep XINWIKI_PUBLISH_TOKEN only on the server (env var).`,
