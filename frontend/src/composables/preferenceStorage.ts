 ...
 *
 * Storage layout: XinWiki_${userId}_${suffix}, where userId is the active
 ...
 *   - Legacy un-namespaced keys (XinWiki_${suffix}) from earlier branch
...
export function readUserId(): string {
  try {
    const raw = localStorage.getItem('xinwiki_user')
    if (!raw) return 'anon'
...
export function safeSetItem(key: string, value: string): void {
  try {
    ...
    console.warn(`[XinWiki] failed to persist preference "${key}":`, err)
  }
}
...
export function userKey(suffix: string): string {
  return `XinWiki_${readUserId()}_${suffix}`
}
...
export function migratePreferencesIntoUser(): void {
  ...
  for (const suffix of PREFERENCE_SUFFIXES) {
    const target = `XinWiki_${userId}_${suffix}`
    const targetExists = safeGetItem(target) !== null

    const anonKey = `XinWiki_anon_${suffix}`
    const legacyKey = `XinWiki_${suffix}`
