...
export const useAuthStore = defineStore('auth', () => {
  ...
  //
  // SECURITY: This value is read from localStorage (`xinwiki_memberships`)
  ...
  const setUser = (userData: UserInfo) => {
    ...
    user.value = userData
    // 保存到localStorage
    localStorage.setItem('xinwiki_user', JSON.stringify(userData))
  ...
  const setTenant = (tenantData: TenantInfo) => {
    ...
    // 保存到localStorage
    localStorage.setItem('xinwiki_tenant', JSON.stringify(tenantData))
  }

  const setToken = (tokenValue: string) => {
    token.value = tokenValue
    localStorage.setItem('xinwiki_token', tokenValue)
  }

  const setRefreshToken = (refreshTokenValue: string) => {
    refreshToken.value = refreshTokenValue
    localStorage.setItem('xinwiki_refresh_token', refreshTokenValue)
  }

  const setKnowledgeBases = (kbList: KnowledgeBaseInfo[]) => {
    // 确保输入是数组
    knowledgeBases.value = Array.isArray(kbList) ? kbList : []
    localStorage.setItem('xinwiki_knowledge_bases', JSON.stringify(knowledgeBases.value))
  }

  const setCurrentKnowledgeBase = (kb: KnowledgeBaseInfo | null) => {
    currentKnowledgeBase.value = kb
    if (kb) {
      localStorage.setItem('xinwiki_current_kb', JSON.stringify(kb))
    } else {
      localStorage.removeItem('xinwiki_current_kb')
    }
  }
  ...
  const clearTenantScopedClientState = () => {
    try {
      localStorage.removeItem('xinwiki_last_chat_model_id')
      localStorage.removeItem('xinwiki_current_kb')
      ...
      if (raw) {
        ...
        if (parsed && typeof parsed === 'object') {
          ...
          localStorage.setItem('XinWiki_settings', JSON.stringify(parsed))
        }
      }
    } catch (e) {
  ...
  const setSelectedTenant = (tenantId: number | null, tenantName: string | null = null) => {
    ...
    if (tenantId !== null) {
      localStorage.setItem('xinwiki_selected_tenant_id', String(tenantId))
      if (tenantName) {
        localStorage.setItem('xinwiki_selected_tenant_name', tenantName)
      }
    } else {
      localStorage.removeItem('xinwiki_selected_tenant_id')
      localStorage.removeItem('xinwiki_selected_tenant_name')
    }
    if (tenantChanged) {
  ...
  const setMemberships = (
    ...
    localStorage.setItem('xinwiki_memberships', JSON.stringify(memberships.value))
  }
  ...
  const setLiteMode = (value: boolean) => {
    ...
    if (value) {
      localStorage.setItem('xinwiki_lite_mode', 'true')
    } else {
      localStorage.removeItem('xinwiki_lite_mode')
    }
  }
  ...
  const logout = () => {
    ...
    // 清空localStorage
    localStorage.removeItem('xinwiki_user')
    localStorage.removeItem('xinwiki_tenant')
    localStorage.removeItem('xinwiki_token')
    localStorage.removeItem('xinwiki_refresh_token')
    localStorage.removeItem('xinwiki_knowledge_bases')
    localStorage.removeItem('xinwiki_current_kb')
    localStorage.removeItem('xinwiki_selected_tenant_id')
    localStorage.removeItem('xinwiki_selected_tenant_name')
    localStorage.removeItem('xinwiki_memberships')
    localStorage.removeItem('xinwiki_lite_mode')
    isLiteMode.value = false
    try {
      sessionStorage.removeItem('xinwiki_lite_last_path')
    } catch {
      /* ignore */
    }
    reloadUserPreferences()
  ...
  const initFromStorage = () => {
    // 从localStorage恢复状态
    const storedUser = localStorage.getItem('xinwiki_user')
    const storedTenant = localStorage.getItem('xinwiki_tenant')
    const storedToken = localStorage.getItem('xinwiki_token')
    const storedRefreshToken = localStorage.getItem('xinwiki_refresh_token')
    const storedKnowledgeBases = localStorage.getItem('xinwiki_knowledge_bases')
    const storedCurrentKb = localStorage.getItem('xinwiki_current_kb')
    const storedSelectedTenantId = localStorage.getItem('xinwiki_selected_tenant_id')
    const storedSelectedTenantName = localStorage.getItem('xinwiki_selected_tenant_name')
    ...
    const storedMemberships = localStorage.getItem('xinwiki_memberships')
    ...
    isLiteMode.value = localStorage.getItem('xinwiki_lite_mode') === 'true'
  }
