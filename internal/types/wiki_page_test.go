...
func TestWikiPageStatusConstants(t *testing.T) {
	statuses := []string{
		WikiPageStatusDraft,
	...
	for _, s := range statuses {
		if s == "" {
			t.Errorf("WikiPageStatus constant is empty")
		}
		if seen[s] {
			t.Errorf("Duplicate WikiPageStatus: %s", s)
		}
		seen[s] = true
	}
}

func TestIsValidWikiPageStatus(t *testing.T) {
	valid := []string{
		WikiPageStatusDraft,
		WikiPageStatusReviewing,
		WikiPageStatusPublished,
	...
	invalid := []string{"", "unknown", "DELETED", "pending", "review"}
	for _, s := range invalid {
		if IsValidWikiPageStatus(s) {
			t.Errorf("%q should NOT be valid status", s)
		}
	}
}

func TestWikiPageStatusTransition(t *testing.T) {
	tests := []struct {
		name      string
		from      string
		to        string
		...
		{"draft -> reviewing", WikiPageStatusDraft, WikiPageStatusReviewing, true},
		{"reviewing -> published", WikiPageStatusReviewing, WikiPageStatusPublished, true},
		{"reviewing -> draft", WikiPageStatusReviewing, WikiPageStatusDraft, true},
		{"published -> deprecated", WikiPageStatusPublished, WikiPageStatusDeprecated, true},
		{"published -> superseded", WikiPageStatusPublished, WikiPageStatusSuperseded, true},
		{"deprecated -> archived", WikiPageStatusDeprecated, WikiPageStatusArchived, true},
		{"superseded -> archived", WikiPageStatusSuperseded, WikiPageStatusArchived, true},
		{"draft -> published", WikiPageStatusDraft, WikiPageStatusPublished, false},
		{"published -> reviewing", WikiPageStatusPublished, WikiPageStatusReviewing, false},
		{"archived -> published", WikiPageStatusArchived, WikiPageStatusPublished, false},
		{"archived -> draft", WikiPageStatusArchived, WikiPageStatusDraft, false},
		{"deprecated -> published", WikiPageStatusDeprecated, WikiPageStatusPublished, false},
		{"superseded -> published", WikiPageStatusSuperseded, WikiPageStatusPublished, false},
		{"draft -> archived", WikiPageStatusDraft, WikiPageStatusArchived, false},
		{"reviewing -> deprecated", WikiPageStatusReviewing, WikiPageStatusDeprecated, false},
		{"same status transition", WikiPageStatusDraft, WikiPageStatusDraft, true},
		{"same status published", WikiPageStatusPublished, WikiPageStatusPublished, true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := ValidateWikiPageStatusTransition(tt.from, tt.to)
			if tt.wantValid && err != nil {
				t.Errorf("Transition %s -> %s should be valid, got error: %v", tt.from, tt.to, err)
			}
			if !tt.wantValid && err == nil {
				t.Errorf("Transition %s -> %s should be invalid, got nil error", tt.from, tt.to)
			}
		})
	}
}

func TestWikiPageCanTransitionTo(t *testing.T) {
	page := &WikiPage{Status: WikiPageStatusDraft}

	if !page.CanTransitionTo(WikiPageStatusReviewing) {
		t.Error("draft should be able to transition to reviewing")
	}
	if page.CanTransitionTo(WikiPageStatusPublished) {
		t.Error("draft should NOT be able to transition directly to published")
	}

	page.Status = WikiPageStatusArchived
	if page.CanTransitionTo(WikiPageStatusPublished) {
		t.Error("archived should NOT be able to transition to published")
	}
}

func TestWikiPageStatusTransitionErrorTypes(t *testing.T) {
	err := ValidateWikiPageStatusTransition(WikiPageStatusArchived, WikiPageStatusPublished)
	if err == nil {
		t.Fatal("Expected error for archived -> published")
	}
	if _, ok := err.(*InvalidWikiStatusTransitionError); !ok {
		t.Errorf("Expected InvalidWikiStatusTransitionError, got %T", err)
	}
}

func TestNormalizeWikiPageStatus(t *testing.T) {
	tests := map[string]string{
		"":           WikiPageStatusPublished,
		"draft":      WikiPageStatusDraft,
		"reviewing":  WikiPageStatusReviewing,
		"published":  WikiPageStatusPublished,
		"deprecated": WikiPageStatusDeprecated,
		"superseded": WikiPageStatusSuperseded,
		"archived":   WikiPageStatusArchived,
		"unknown":    WikiPageStatusPublished,
		"  draft  ":  WikiPageStatusDraft,
	}
	for in, want := range tests {
		if got := NormalizeWikiPageStatus(in); got != want {
			t.Errorf("NormalizeWikiPageStatus(%q) = %q, want %q", in, got, want)
		}
	}
}
