...
// UserSecurityLevelFromContext extracts the user's knowledge security level
// clearance from ctx. Returns SecurityLevelL1 (least privileged) when the key
// is absent, so callers fail closed if the auth middleware did not populate it.
func UserSecurityLevelFromContext(ctx context.Context) string {
	v, ok := ctx.Value(UserSecurityLevelContextKey).(string)
...
// UserGroupIDsFromContext extracts the user's group membership IDs from ctx.
// Returns nil when the key is absent.
func UserGroupIDsFromContext(ctx context.Context) []string {
	v, ok := ctx.Value(UserGroupIDsContextKey).([]string)
	if !ok {
		return nil
	}
	return v
