...
const (
	// SecurityLevelL1 is the default, least-restrictive knowledge security level.
	SecurityLevelL1 = "L1"
	// SecurityLevelL2 represents internal / restricted knowledge.
	SecurityLevelL2 = "L2"
	// SecurityLevelL3 represents confidential knowledge.
	SecurityLevelL3 = "L3"
	// SecurityLevelL4 represents highly confidential knowledge.
	SecurityLevelL4 = "L4"
)

// ACLSource represents a single source of ACL metadata used for derived
// knowledge permission propagation.
...
// DerivedACLResult holds the computed ACL metadata for derived knowledge
// (wiki pages, caches, reports, agent outputs).
type DerivedACLResult struct {
	SecurityLevel   string
	AllowedUserIDs  []string
	AllowedGroupIDs []string
}
...
type Chunk struct {
	...
	// SecurityLevel records the chunk-level classification used by derived
	// knowledge ACL propagation. Empty legacy values are normalized to L1.
	SecurityLevel string `json:"security_level"            gorm:"type:varchar(16);not null;default:'L1';index"`
	// AllowedUserIDs optionally narrows this chunk to specific users. An empty
	// list does not mean public; callers must still apply tenant / KB RBAC.
	AllowedUserIDs StringArray `json:"allowed_user_ids"          gorm:"type:json"`
	// AllowedGroupIDs optionally narrows this chunk to specific groups. An
	// empty list does not mean public; callers must still apply tenant / KB RBAC.
...
// NormalizeACLDefaults applies safe defaults for chunk-level ACL metadata.
// Empty ACL arrays are intentionally not interpreted as public access; they
// mean callers must continue to enforce tenant / KB RBAC at runtime.
func (c *Chunk) NormalizeACLDefaults() {
	if c == nil {
		return
	}
	if strings.TrimSpace(c.SecurityLevel) == "" {
		c.SecurityLevel = SecurityLevelL1
	}
	if c.AllowedUserIDs == nil {
		c.AllowedUserIDs = StringArray{}
	}
	if c.AllowedGroupIDs == nil {
		c.AllowedGroupIDs = StringArray{}
	}
}
