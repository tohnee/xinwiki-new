...
import (
	...
	modelsutils "github.com/Tencent/XinWiki/internal/models/utils"
...
// NewXinWikiCloudService 构造 XinWikiCloudService
func NewXinWikiCloudService(
	repo interfaces.ModelRepository,
	tenantRepo interfaces.TenantRepository,
) interfaces.XinWikiCloudService {
	return &weKnoraCloudService{
		tenantRepo: tenantRepo,
	}
}

func IsXinWikiCloudDocReaderAddr(addr string) bool {
	return strings.TrimSuffix(strings.TrimSpace(addr), "/") == strings.TrimRight(provider.XinWikiCloudBaseURL, "/")+"/api/v1/doc/reader"
}
...
// verifyCredentials 向 XinWikiCloud /api/v1/health 发送带签名头的 GET。
//
...
func (s *weKnoraCloudService) verifyCredentials(ctx context.Context, appID, appSecret string) error {
	baseURL := strings.TrimRight(provider.XinWikiCloudBaseURL, "/")
...
// CheckStatus 检查 XinWikiCloud 凭证是否可正常解密
func (s *weKnoraCloudService) CheckStatus(ctx context.Context) (*types.XinWikiCloudStatusResult, error) {
	tenantID := types.MustTenantIDFromContext(ctx)

	tenant, err := s.tenantRepo.GetTenantByID(ctx, tenantID)
	if err != nil || tenant == nil {
		return &types.XinWikiCloudStatusResult{HasModels: false, NeedsReinit: false}, nil
	}

	creds := tenant.Credentials.GetXinWikiCloud()
	if creds == nil {
		return &types.XinWikiCloudStatusResult{HasModels: false, NeedsReinit: false}, nil
	}
	...
	if strings.HasPrefix(creds.AppSecret, utils.EncPrefix) {
		return &types.XinWikiCloudStatusResult{
			HasModels:   true,
			NeedsReinit: true,
			Reason:      "XinWikiCloud 凭证解密失败（服务重启后加密密钥已变更），请重新填写 APPID 和 APPSECRET",
		}, nil
	}

	return &types.XinWikiCloudStatusResult{HasModels: true, NeedsReinit: false}, nil
}

// updateTenantCredentials 更新租户的 XinWikiCloud 凭证
func (s *weKnoraCloudService) updateTenantCredentials(ctx context.Context, tenantID uint64, appID, appSecret string) error {
	...
	tenant.Credentials.XinWikiCloud = &types.XinWikiCloudCredentials{
		AppID:     appID,
		AppSecret: appSecret,
	}
