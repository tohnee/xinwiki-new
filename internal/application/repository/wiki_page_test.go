...
CREATE TABLE IF NOT EXISTS wiki_pages (
    ...
    security_level    VARCHAR(16) NOT NULL DEFAULT 'L1',
...
func TestWikiPage_ACLMetadataRoundTripAndDefaults(t *testing.T) {
	db := setupWikiPagesTestDB(t)
	repo := NewWikiPageRepository(db)
	ctx := context.Background()

	page := makeWikiPage("kb-acl", "entity/secure", types.WikiPageTypeEntity, types.WikiPageStatusDraft)
	page.SecurityLevel = types.SecurityLevelL3
	page.AllowedUserIDs = types.StringArray{"user-a", "user-b"}
	page.AllowedGroupIDs = types.StringArray{"group-red"}
	require.NoError(t, repo.Create(ctx, page))

	got, err := repo.GetBySlug(ctx, "kb-acl", "entity/secure")
	require.NoError(t, err)
	assert.Equal(t, types.SecurityLevelL3, got.SecurityLevel)
	assert.Equal(t, types.StringArray{"user-a", "user-b"}, got.AllowedUserIDs)
	assert.Equal(t, types.StringArray{"group-red"}, got.AllowedGroupIDs)

	legacy := makeWikiPage("kb-acl", "entity/legacy", types.WikiPageTypeEntity, types.WikiPageStatusPublished)
	require.NoError(t, repo.Create(ctx, legacy))

	gotLegacy, err := repo.GetBySlug(ctx, "kb-acl", "entity/legacy")
	require.NoError(t, err)
