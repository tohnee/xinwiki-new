...
func TestCreateChunks_SQLite_ACLMetadataRoundTripAndDefaults(t *testing.T) {
	...
	secure := makeChunk(kbID, knowledgeID, "text")
	secure.SecurityLevel = types.SecurityLevelL3
	secure.AllowedUserIDs = types.StringArray{"user-a", "user-b"}
	secure.AllowedGroupIDs = types.StringArray{"group-red"}
	legacy := makeChunk(kbID, knowledgeID, "text")

	require.NoError(t, repo.CreateChunks(ctx, []*types.Chunk{secure, legacy}))

	gotSecure, err := repo.GetChunkByID(ctx, 1, secure.ID)
	require.NoError(t, err)
	assert.Equal(t, types.SecurityLevelL3, gotSecure.SecurityLevel)
	assert.Equal(t, types.StringArray{"user-a", "user-b"}, gotSecure.AllowedUserIDs)
	assert.Equal(t, types.StringArray{"group-red"}, gotSecure.AllowedGroupIDs)

	gotLegacy, err := repo.GetChunkByID(ctx, 1, legacy.ID)
	require.NoError(t, err)
	assert.Equal(t, types.SecurityLevelL1, gotLegacy.SecurityLevel)
	assert.Empty(t, gotLegacy.AllowedUserIDs)
	assert.Empty(t, gotLegacy.AllowedGroupIDs)
}

func TestCreateChunks_SQLite_ACLDefaultsDoNotMeanPublic(t *testing.T) {
	chunk := makeChunk("kb", "knowledge", "text")
	chunk.NormalizeACLDefaults()

	assert.Equal(t, types.SecurityLevelL1, chunk.SecurityLevel)
	assert.NotNil(t, chunk.AllowedUserIDs)
	assert.Empty(t, chunk.AllowedUserIDs)
	assert.NotNil(t, chunk.AllowedGroupIDs)
	assert.Empty(t, chunk.AllowedGroupIDs)
}
