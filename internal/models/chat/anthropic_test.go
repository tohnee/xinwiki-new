...
func TestAnthropicChat(t *testing.T) {
	...
	contentBlocks, ok := capturedRequest.Messages[0].Content.([]interface{})
	require.True(t, ok, "expected content to be []interface{}, got %T", capturedRequest.Messages[0].Content)
	require.Len(t, contentBlocks, 1)
	block, ok := contentBlocks[0].(map[string]interface{})
	require.True(t, ok, "expected block to be map[string]interface{}")
