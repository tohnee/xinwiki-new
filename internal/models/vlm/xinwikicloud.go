...
// XinWikiCloudVLM implements VLM via the XinWikiCloud API.
type XinWikiCloudVLM struct {
	modelName       string
...
// NewXinWikiCloudVLM creates a XinWikiCloud-backed VLM instance.
func NewXinWikiCloudVLM(config *Config) (*XinWikiCloudVLM, error) {
	if config.AppID == "" {
		return nil, fmt.Errorf("XinWikiCloud VLM: AppID is required")
	}
	if config.AppSecret == "" {
		return nil, fmt.Errorf("XinWikiCloud VLM: AppSecret is required")
	}
	remoteModelName := ""
	if config.Extra != nil {
...
// Predict sends images with a text prompt to the XinWikiCloud API.
func (v *XinWikiCloudVLM) Predict(ctx context.Context, imgBytesList [][]byte, prompt string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("xinwikicloud VLM: marshal: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("xinwikicloud VLM: create request: %w", err)
	}
	...
	logger.Infof(ctx, "[VLM] Calling XinWikiCloud API, model=%s, baseURL=%s, numImages=%d, totalImageSize=%d",
		v.effectiveModelName(), v.baseURL, len(imgBytesList), totalImageSize)

	resp, err := v.client.Do(req)
	if err != nil {
		return "", fmt.Errorf("xinwikicloud VLM: do request: %w", err)
	}
	defer resp.Body.Close()

	respBytes, err := io.ReadAll(resp.Body)
	if err != nil {
		return "", fmt.Errorf("xinwikicloud VLM: read response: %w", err)
	}
	if resp.StatusCode != http.StatusOK {
		return "", fmt.Errorf("xinwikicloud VLM: status %d: %s", resp.StatusCode, string(respBytes))
	}

	var vlmResp weKnoraCloudVLMResponse
	if err := json.Unmarshal(respBytes, &vlmResp); err != nil {
		return "", fmt.Errorf("xinwikicloud VLM: unmarshal: %w", err)
	}
	if len(vlmResp.Choices) == 0 {
		return "", fmt.Errorf("xinwikicloud VLM: no choices in response")
	}

	content := vlmResp.Choices[0].Message.Content
	logger.Infof(ctx, "[VLM] XinWikiCloud response received, len=%d", len(content))
	return content, nil
}

func (v *XinWikiCloudVLM) effectiveModelName() string {
...
func (v *XinWikiCloudVLM) GetModelName() string { return v.modelName }
func (v *XinWikiCloudVLM) GetModelID() string   { return v.modelID }
