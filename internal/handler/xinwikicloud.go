...
// XinWikiCloudHandler 处理 XinWikiCloud 凭证管理
type XinWikiCloudHandler struct {
	svc interfaces.XinWikiCloudService
}

// NewXinWikiCloudHandler 构造函数
func NewXinWikiCloudHandler(svc interfaces.XinWikiCloudService) *XinWikiCloudHandler {
	return &XinWikiCloudHandler{svc: svc}
}
...
// SaveCredentials POST /api/v1/xinwikicloud/credentials
...
// @Security     Bearer
// @Security     ApiKeyAuth
// @Router       /xinwikicloud/credentials [post]
func (h *XinWikiCloudHandler) SaveCredentials(c *gin.Context) {
...
// Status GET /api/v1/models/xinwikicloud/status
// 检查当前租户的 XinWikiCloud 凭证是否完好，如需重新初始化则返回 needs_reinit=true
//
// Status godoc
...
// @Security     Bearer
// @Security     ApiKeyAuth
// @Router       /models/xinwikicloud/status [get]
func (h *XinWikiCloudHandler) Status(c *gin.Context) {
