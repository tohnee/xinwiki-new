...
echo "[cleanup] 1/8 停止 XinWiki 容器"
COMPOSE_PROJECT=""
if [[ -d "${XINWIKI_DIR}" ]]; then
  cd "${XINWIKI_DIR}"
  # 优先用 compose ls 拿到真实 project 名 (默认是目录名小写, 如 xinwiki)
  COMPOSE_PROJECT="$(docker compose ls --format json 2>/dev/null \
    | grep -oE '"Name":"[^"]+"' | head -1 | cut -d'"' -f4 || true)"
  docker compose down -v --remove-orphans || true
fi

echo "[cleanup] 2/8 清空 XinWiki 业务数据 + 首启 marker / 日志"
if [[ -d "${XINWIKI_DIR}" ]]; then
  rm -rf "${XINWIKI_DIR}/data"/* "${XINWIKI_DIR}/logs"/* 2>/dev/null || true
  ...
  rm -f "${XINWIKI_DIR}/.env" "${XINWIKI_DIR}/.firstboot.done"
fi
rm -f /root/xinwiki-credentials.txt /var/log/xinwiki-firstboot.log
...
# wechatopenai/xinwiki-* 等镜像一并清掉, 导致基于镜像建出来的新实例
