...
XINWIKI_BASE_URL = os.getenv("XINWIKI_BASE_URL", "http://localhost:8080/api/v1")
XINWIKI_API_KEY = os.getenv("XINWIKI_API_KEY", "")
# Chat SSE read timeout in seconds. LLM responses can be slow; default 300s.
try:
    XINWIKI_CHAT_TIMEOUT = int(os.getenv("XINWIKI_CHAT_TIMEOUT", "300"))
except ValueError:
    logger.warning("XINWIKI_CHAT_TIMEOUT is not a valid integer; falling back to 300s.")
    XINWIKI_CHAT_TIMEOUT = 300


class XinWikiClient:
    """Client for interacting with XinWiki API"""

    def __init__(self, base_url: str, api_key: str):
        """Initialize the XinWiki API client with base URL and authentication"""
        self.base_url = base_url
        self.api_key = api_key
        # SSL verification: enabled by default. Set XINWIKI_VERIFY_SSL=false to disable
        # (e.g. for self-signed certs in dev environments — NOT recommended for production).
        self.verify_ssl = os.getenv("XINWIKI_VERIFY_SSL", "true").lower() != "false"
        if not self.verify_ssl:
            logger.warning(
                "SSL certificate verification is DISABLED (XINWIKI_VERIFY_SSL=false). "
    ...
    def _request(self, method: str, endpoint: str, **kwargs) -> Dict[str, Any]:
        """Make a request to the XinWiki API
    ...
    def _consume_sse_stream(self, url: str, body: Dict[str, Any]) -> Dict:
        ...
        Timeout: (10s connect, XINWIKI_CHAT_TIMEOUT read) — configurable via env var.
        ...
        try:
            ...
            # Timeout: 10s to establish connection, XINWIKI_CHAT_TIMEOUT for reading response
            response = self.session.post(
                url, json=body, stream=True,
                timeout=(10, XINWIKI_CHAT_TIMEOUT),
            )
            response.raise_for_status()
...
app = Server("xinwiki-server")
# Initialize XinWiki API client with configuration
client = XinWikiClient(XINWIKI_BASE_URL, XINWIKI_API_KEY)


# Tool definitions - Register all available tools for the MCP protocol
@app.list_tools()
async def handle_list_tools() -> list[types.Tool]:
    """List all available XinWiki tools with their schemas"""
    return [
        # Tenant Management
        types.Tool(
            name="create_tenant",
            description="Create a new tenant in XinWiki",
            inputSchema={
                "type": "object",
                "properties": {
...
def main():
    ...
    Transport selection (in priority order):
      ...
      3. Default: stdio
    """
    parser = argparse.ArgumentParser(description="XinWiki MCP Server")
    parser.add_argument(
        "--transport",
