# XinWiki Mini Program

This directory contains a WeChat Mini Program plugin for XinWiki. It gives mobile users a lightweight entry point to:

- configure a XinWiki API endpoint and tenant API key;
- list available knowledge bases;
- import a URL into a selected knowledge base;
- ask a selected knowledge base through XinWiki knowledge chat.
...
- WeChat DevTools may block `localhost` requests when URL validation is enabled. For local testing, either disable domain validation in DevTools or expose XinWiki through a HTTPS development domain.
- In production Mini Programs, add the XinWiki API domain to the Mini Program request domain allowlist.
