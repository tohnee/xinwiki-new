...
- 改名 XinWiki 到 XinWiki 只改用户可见层，例如前端标题、文档、README、部署展示名；后端 Go 包路径、`XINWIKI_*` 配置前缀、迁移与内部标识保持不变，避免大范围 diff 与迁移风险。
...
XinWiki 当前最重要的工作不是把企业能力从零再做一遍，而是在现有 XinWiki 能力之上补齐“知识资产治理语义”。
