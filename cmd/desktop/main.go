...
import (
	...
	"github.com/Tencent/XinWiki/internal/container"
	"github.com/Tencent/XinWiki/internal/logger"
	"github.com/Tencent/XinWiki/internal/runtime"
...
func main() {
	...
	// Initialize the XinWiki App struct
	app := NewApp()
	...
	FileMenu := AppMenu.AddSubmenu("XinWiki Lite")
	FileMenu.AddText("About XinWiki", keys.CmdOrCtrl("i"), func(_ *menu.CallbackData) {
		if app.ctx == nil {
			return
		}
		...
		if choice == "Open GitHub" {
			wailsruntime.BrowserOpenURL(app.ctx, xinwikiGitHubRepoURL)
		}
	})
...
func configureDesktopStorage(execPath string) {
	...
	dbPath := resolveDesktopDataPath(os.Getenv("DB_PATH"), filepath.Join("data", "xinwiki.db"), appSupportDir)
