# Changelog — `xinwiki` CLI

All notable changes to the `xinwiki` CLI (the binary under `cli/` in this
...
of the XinWiki server / frontend release cadence.
...
#### Added
- `xinwiki session stop <session-id>` command to abort an in-flight agent run.
...
- **`auth login` drops `--host` and `--name`.** It authenticates the active profile; create it first with `xinwiki profile add <name> --host <h> --use`. Target a non-active profile with the global `xinwiki --profile <name> auth login`.
...
#### Added
- `--dry-run` flag on every mutation cobra command (`kb create/edit/delete`, `agent create/edit/delete`, `doc create/upload/fetch/delete`, `chunk delete`, `session delete`, `auth refresh/logout`, `link/unlink`, `profile add/remove`) and on `xinwiki api` (POST/PUT/PATCH/DELETE only; GET returns FlagError exit 2).
- envelope `meta.dry_run: true` + `meta.plan: {action, args | method+path+body}` open-map fields (omitempty in non-dry-run envelopes).
- `xinwiki session continue-stream <session-id> --message <msg-id>` command for SSE event stream replay/recovery.
...
#### Changed
- `SetAgentHelp` wrapper in `cli/internal/cmdutil/agenthelp.go` now prepends "Risk:" line in default (non-JSON) help branch when `cmd.Annotations["risk.action"]` is set. XINWIKI_AGENT_HELP=1 JSON path unchanged.
...
- **`--format` default flips to `json` regardless of TTY.**
  ...
  - Migration: humans on a TTY pass `--format text` (or set
    `XINWIKI_FORMAT=text` env) for the prior auto-text behavior.
...
- **`xinwiki agent invoke` removed; use `xinwiki session ask --agent <id>`.**
  - Server route is `POST /sessions/{session_id}/agent-qa` — session-anchored.
  - `xinwiki agent` keeps CRUD only (list / view / create / edit / delete /
    status / check).
  - Migration: `xinwiki agent invoke ag_x "Q"` →
    `xinwiki session ask --agent ag_x "Q"` (auto-creates session if none given).
- **`xinwiki doc upload` split into three commands.**
  - `xinwiki doc upload <file>` — local file only.
  - `xinwiki doc fetch <url>` — server-side remote fetch (was `upload --from-url`).
  - `xinwiki doc create --text "..."` — direct text knowledge.
...
- **`xinwiki kb empty` removed; use `xinwiki doc delete --all --kb=<id>`.**
  - Atomic server `ClearKnowledgeBaseContents` (no list-then-delete race).
  - Same exit-10 `-y/--yes` guard as `kb delete`.
  - Migration: `xinwiki kb empty kb_x -y` →
    `xinwiki doc delete --all --kb=kb_x -y`.
- **`xinwiki api -d/--data` flag removed; use `--input <file>` or `--input -`
  (stdin).**
  - `xinwiki api` now accepts any non-empty HTTP method (whitelist removed)
    so the escape hatch can hit endpoints the CLI doesn't natively model.
  - Migration: `xinwiki api -d '{"foo":1}' /endpoint` →
    `echo '{"foo":1}' | xinwiki api --input - /endpoint`.
- **Batch operations envelope shape — per-item `ok` pattern.**
  - `xinwiki doc delete id1,id2,id3` and similar multi-id mutations now emit:
...
- **Unknown subcommand emits typed envelope.**
  - `input.unknown_subcommand` with `detail.{unknown, command_path, available[]}`
    ...
    `"unknown command \"x\" for \"xinwiki\""` prose.
- **`xinwiki chat` requires the query as a single quoted argument.**
  - v0.6: `MinimumNArgs(1)` silently joined `xinwiki chat hello world` into
    `"hello world"`.
  - v0.7: `ExactArgs(1)` rejects multi-arg with exit 2; matches
    `xinwiki session ask`. Quote the query: `xinwiki chat "hello world"`.

#### Added
- **`XINWIKI_PROFILE` env var** selects the active profile for a single
...
- **`XINWIKI_FORMAT` env var** sets the default `--format`. Values:
...
- **`error.retry_after_seconds`** — `server.rate_limited` / `server.timeout`
  surface server `Retry-After` header verbatim. CLI-direct (`xinwiki api`)
...
- **`xinwiki doc fetch <url>`** — new command (see split above).
- **`xinwiki doc create --text "..."`** — new command (see split above).
- **`xinwiki session ask --agent <id> "..."`** — new command (replaces
  `agent invoke`).
- **`xinwiki doc delete --all --kb=<id>`** — new mode of `doc delete`
  (replaces `kb empty`).
...
- **`XINWIKI_SDK_DEBUG=1` env removed** → use **`XINWIKI_LOG_LEVEL=debug`**.
...
- **`xinwiki doc wait <id> [<id>...]`** — block until every document reaches a
...
- **`--log-level error|warn|info|debug`** persistent flag + `XINWIKI_LOG_LEVEL`
...
- **`xinwiki kb status <id>`** — fast health snapshot (1 HTTP). Returns
  reachable / counts / is_processing.
- **`xinwiki kb check <id>`** — deep verification: status fields + `failed_count`
...
- **`SetAgentHelp` pattern** — `cmdutil.SetAgentHelp(cmd, AgentHelp{...})`
  ...
  shape, activated by `XINWIKI_AGENT_HELP=1` at `--help` time. Applied
...
#### Added
- `xinwiki agent create <name> --model <id>` / `agent edit <id>` /
...
- `xinwiki chunk list --doc <doc-id>` / `chunk view <chunk-id>` /
  `chunk delete <chunk-id> --doc <doc-id>` — new subtree for RAG retrieval
  debug. Paginated with v0.4 `--limit` / `--page-size` / `--all-pages` canon.
- `xinwiki mcp serve` adds `chunk_list` as the 10th curated tool.
- `xinwiki agent view <id>` human output now renders all 34 AgentConfig
...
- `xinwiki doc list` gains `--keyword` / `--file-type` / `--source` /
...
- `xinwiki session view --full` (with `--limit`, default 50, bounds
...
- `xinwiki kb view` human render now includes `TYPE`, `PINNED` (badge,
...
- `xinwiki doc view` human render expands to include `TITLE` (when
...
- `xinwiki doc upload` gains `--enable-multimodel` (tri-state:
...
#### Breaking changes
- `xinwiki search docs` now applies the keyword filter server-side via
  ...
  **The match is now case-sensitive** (server uses `LIKE %keyword%`),
  ...
  back to `xinwiki api` with a custom filter.
...
- `cli/README.md` now documents the `xinwiki api` raw HTTP passthrough
...
#### Added
- `xinwiki mcp serve` — curated read-only stdio MCP server exposing 9
...
- `xinwiki agent list` / `agent view` / `agent invoke` — manage and
  call XinWiki's server-side Custom Agent resources.
- `xinwiki auth token` — print the active credential to `stdout` for
  scripting (raw secret by default; `--json` emits `{token, mode, context}`).
- `xinwiki doc upload --from-url` — ingest a remote URL.
...
- `auth login --with-token` validates the supplied key against
  ...
  `$XDG_CONFIG_HOME/xinwiki/secrets/`.
...
- `unlink` — remove the cwd's `.xinwiki/project.yaml` so subsequent
