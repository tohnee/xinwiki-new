---
name: xinwiki-shared
description: Use when driving a XinWiki RAG server through the `xinwiki` CLI as an agent — authenticating, managing knowledge bases / documents / sessions / agents, running search or chat, or interpreting the CLI's JSON envelopes and exit codes. Read this before any other xinwiki-* skill.
metadata:
  tested_against: v0.9
---

# XinWiki CLI — shared base

`xinwiki` is the agent-first CLI for a XinWiki RAG server. Every command prints
a JSON envelope and uses a typed exit code, so you branch on machine-readable
output, not prose. Read this skill before any task-specific `xinwiki-*` skill.

## 1. Authenticate (do this first — order matters)

Authentication is a **two-step sequence**. `xinwiki auth login` operates on the
...
echo "$XINWIKI_API_KEY" | xinwiki auth login --with-token
# 2b. OR email+password (interactive prompt only — no flags; not for agents)
xinwiki auth login

xinwiki auth status          # verify: who am I, which tenant
```

- Target a *non-active* profile for one command with the global `--profile NAME`
  (e.g. `xinwiki --profile staging auth refresh`). There is no per-command
  `--name`/`--host` on auth commands.
- Get the raw token for scripting with `XINWIKI_TOKEN=$(xinwiki auth token)`
  (raw token by default; `--format json` gives the `{token, mode, profile}` envelope).
- `xinwiki doctor` runs 4 health checks (reachability, credential, version, storage).

## 2. Selecting a knowledge base (`--kb`)

`--kb` accepts a **name or a UUID** (resolved server-side). Resolution order:
`--kb` flag → `XINWIKI_KB_ID` env → directory link (`xinwiki link --kb X` binds
...
- `--jq '<expr>'` filters the envelope (e.g. `xinwiki kb list --jq '.data[].id'`).
- Exception: `xinwiki auth token` emits the **raw token** by default (it's a
...
| 2 | flag / argument validation (bad/unknown/missing-required flag) | re-check `xinwiki <cmd> --help` |
...
{"ok":false,"error":{"type":"input.confirmation_required",
  ...
  "retry_command":"xinwiki kb delete X -y","risk":{"level":"destructive","action":"kb.delete"}}}
```
...
```bash
KB=$(xinwiki kb create "Docs" --jq '.data.id' --format json | tr -d '"')
xinwiki doc upload ./manual.pdf --kb "$KB"
```
...
link/unlink  bind cwd to a KB        mcp serve  expose xinwiki as MCP tools
```

**chat vs session ask vs search** — the most common confusion: see the
`xinwiki-rag-search` skill for the decision table. Briefly: `chat` = one-shot
...
For an IDE/host agent that speaks MCP, `xinwiki mcp serve` exposes a curated
...
Run `xinwiki <command> --help`. With `XINWIKI_AGENT_HELP=1` set, `--help` emits a
...
| Mistake | Fix |
|---|---|
| `xinwiki auth login` before any profile exists | `profile add <n> --host <url> --use` first (§1) |
...
| `xinwiki chat "..."` prints "garbled" stream | NDJSON by default; use `--format text` for a transcript |
| `search chunks "q"` → exit 1 `local.kb_id_required` | Pass `--kb <name-or-id>`, set `XINWIKI_KB_ID`, or `xinwiki link` the dir |
