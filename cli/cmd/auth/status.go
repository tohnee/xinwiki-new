...
// NewCmdStatus builds the `xinwiki auth status` command.
...
` + "`xinwiki auth login`" + ` (or ` + "`auth refresh`" + ` for JWT profiles) to recover.
...
usually only surfaces a hard auth failure.`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{"xinwiki auth status", "xinwiki auth status --jq .data.tenant_id"},
...
func runStatus(ctx context.Context, fopts *cmdutil.FormatOptions, f *cmdutil.Factory, svc StatusService) error {
	if svc == nil {
		return cmdutil.NewError(cmdutil.CodeAuthUnauthenticated, "no SDK client available; run `xinwiki auth login`")
	}
