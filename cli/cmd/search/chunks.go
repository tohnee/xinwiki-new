...
// NewCmdChunks builds `xinwiki search chunks "<query>" --kb <id-or-name>`.
...
func NewCmdChunks(f *cmdutil.Factory) *cobra.Command {
  ...
  xinwiki search chunks "embedding model" --kb kb_abc --limit 20
  xinwiki search chunks "retry policy" --kb engineering --no-keyword  # vector-only`,
