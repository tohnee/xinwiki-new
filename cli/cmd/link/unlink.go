...
// NewCmdUnlink builds `xinwiki unlink`. Walks up from cwd so a user in
...
func NewCmdUnlink() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Removes the .xinwiki/project.yaml that ` + "`xinwiki link`" + ` previously
...
is present anywhere in the parent chain.`,
		Example: `  xinwiki unlink           # remove the binding for this project
  xinwiki unlink --format json    # bare JSON (CI / agents)`,
		Args: cobra.NoArgs,
		RunE: func(c *cobra.Command, _ []string) error {
			...
			if !found {
				return &cmdutil.Error{
					...
					Hint:    "run `xinwiki link --kb <id>` first, or check you're in the right directory",
				}
			}
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		UsedFor: "Remove the .xinwiki/project.yaml KB binding from the current directory tree. No flags required; walks up from cwd to find the link.",
