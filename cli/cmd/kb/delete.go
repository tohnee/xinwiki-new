...
// NewCmdDelete builds `xinwiki kb delete`. The global -y/--yes persistent
...
exactly to guard against unintended deletes.`,
		Example: `  xinwiki kb delete kb_abc                      # interactive confirm
  xinwiki kb delete kb_abc -y                   # no prompt
  xinwiki kb delete kb_abc -y --format json     # bare {id, deleted:true} JSON`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{
			"xinwiki kb delete kb_abc -y",
			"xinwiki kb delete kb_abc -y --format json",
		},
		Warnings: []string{
...
func runDelete(ctx context.Context, opts *DeleteOptions, fopts *cmdutil.FormatOptions, svc DeleteService, p prompt.Prompter, id string) error {
	if err := cmdutil.ConfirmDestructive(p, opts.Yes, fopts.WantsJSON(), "delete", "knowledge base", id, "kb.delete", "xinwiki kb delete "+id+" -y"); err != nil {
		return err
	}
