...
// NewCmdDelete builds `xinwiki session delete`. Single-id keeps the simpler
...
without the user's explicit go-ahead.`,
		Example: `  xinwiki session delete s_abc                  # interactive confirm
  xinwiki session delete s_abc -y               # no prompt
  xinwiki session delete s_abc -y --format json # bare {id, deleted:true} JSON
  xinwiki session delete s_a s_b s_c -y         # delete 3, keep-going`,
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			if err := cmdutil.ConfirmDestructiveBatch(f.Prompter(), opts.Yes, fopts.WantsJSON(), "delete", "session", len(args), "session.delete", "xinwiki session delete "+strings.Join(args, " ")+" -y"); err != nil {
				return err
			}
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{
			"xinwiki session delete s_abc -y",
			"xinwiki session delete s_a s_b s_c -y",
			"xinwiki session delete s_abc -y --format json",
		},
		Warnings: []string{
...
func runDelete(ctx context.Context, opts *DeleteOptions, fopts *cmdutil.FormatOptions, svc DeleteService, p prompt.Prompter, id string) error {
	if err := cmdutil.ConfirmDestructive(p, opts.Yes, fopts.WantsJSON(), "delete", "session", id, "session.delete", "xinwiki session delete "+id+" -y"); err != nil {
		return err
	}
