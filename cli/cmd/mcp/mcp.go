// Package mcpcmd holds the `xinwiki mcp` command tree.
//
...
// `xinwiki mcp serve` exposes a curated subset of the CLI as MCP tools so
// an IDE-side agent can list / view / search / chat against the user's
// active XinWiki profile without shelling out to the CLI per call. Most
...
// NewCmd builds the `xinwiki mcp` parent. Called from cli/cmd/root.go.
func NewCmd(f *cmdutil.Factory) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "mcp",
		Short: "Run xinwiki as a Model Context Protocol server",
		Long: `Exposes xinwiki's tool surface as MCP tools so any
