...
const agentDeleteExample = `  xinwiki agent delete ag_abc           # interactive confirm
  xinwiki agent delete ag_abc -y        # no prompt
  xinwiki agent delete ag_abc -y --format json # bare {id, deleted:true} JSON`

// NewCmdDelete builds `xinwiki agent delete <agent-id>`.
func NewCmdDelete(f *cmdutil.Factory) *cobra.Command {
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{
			"xinwiki agent delete ag_abc -y",
			"xinwiki agent delete ag_abc -y --format json",
		},
		Warnings: []string{
...
func runDelete(ctx context.Context, opts *DeleteOptions, fopts *cmdutil.FormatOptions, svc DeleteService, p prompt.Prompter) error {
	if err := cmdutil.ConfirmDestructive(p, opts.Yes, fopts.WantsJSON(), "delete", "agent", opts.AgentID, "agent.delete", "xinwiki agent delete "+opts.AgentID+" -y"); err != nil {
		return err
	}
