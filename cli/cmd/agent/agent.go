// Package agentcmd holds the `xinwiki agent` command tree:
...
//
// "agent" in this subtree refers to XinWiki's user-defined Custom
// Agents (server resource: GET/POST /agents/...) and handles CRUD
// operations only. Agent invocation has moved to `xinwiki session ask
...
// NewCmd builds the `xinwiki agent` parent and registers leaves. Called
...
or delete agents. To invoke an agent, use: xinwiki session ask --agent <id>`,
	}
