...
// NewCmdView builds `xinwiki agent view <agent-id>`.
...
Use --jq to project specific fields or reach into nested config.`,
		Example: `  xinwiki agent view ag_abc
  xinwiki agent view ag_abc --format json --jq '{id, name, config}'   # top-level projection
  xinwiki agent view ag_abc --format json --jq '.config.system_prompt'`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples:      []string{"xinwiki agent view agent_abc"},
