...
// at `xinwiki auth login` - covers the broad surface (auth status / kb
...
func defaultHint(code ErrorCode) string {
	...
	case CodeAuthUnauthenticated, CodeAuthBadCredential:
		return "run `xinwiki auth login`"
	case CodeAuthTokenExpired:
		return "your session expired; run `xinwiki auth login` to re-authenticate"
	...
	case CodeAuthCrossTenantBlocked, CodeAuthTenantMismatch:
		return "verify tenant profile with `xinwiki auth status`"
	case CodeNetworkError:
		return "check base URL reachability with `xinwiki doctor`"
	case CodeServerIncompatibleVersion:
		return "run `xinwiki doctor` to see version skew details"
	case CodeServerRateLimited:
		return "rate-limited; retry after a few seconds"
	case CodeServerTimeout:
		return "request timed out; retry, or run `xinwiki doctor` to check connectivity"
	case CodeResourceNotFound:
		return "verify the resource ID and try again"
	case CodeInputInvalidArgument, CodeInputMissingFlag:
		return "see `xinwiki <command> --help` for valid usage"
	...
	case CodeLocalConfigCorrupt:
		return "remove ~/.config/xinwiki/config.yaml and re-run `xinwiki auth login`"
	case CodeLocalFileIO:
		return "check file permissions under $XDG_CONFIG_HOME/xinwiki/"
	case CodeKBIDRequired:
		return "run `xinwiki link` to bind this directory to a knowledge base, or pass --kb"
	case CodeKBNotFound:
		return "list available with `xinwiki kb list`"
	case CodeProjectLinkCorrupt:
		return "remove .xinwiki/project.yaml and run `xinwiki link` again"
...
func defaultRetryCommand(code ErrorCode) string {
	...
	case CodeAuthUnauthenticated, CodeAuthBadCredential, CodeAuthTokenExpired:
		return "xinwiki auth login"
	case CodeKBIDRequired:
		return "xinwiki link"
	case CodeNetworkError, CodeServerTimeout:
		return "xinwiki doctor"
	case CodeProjectLinkCorrupt:
		return "xinwiki link" // re-bind the project to a KB
	case CodeLocalConfigCorrupt:
