...
func TestDiscover_FoundInCwd(t *testing.T) {
	...
	want := filepath.Join(dir, ".xinwiki", "project.yaml")
	if got != want {
...
func TestLoad_RoundTrip(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, ".xinwiki", "project.yaml")
...
func TestSave_CreatesParent(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".xinwiki")); err != nil {
		t.Errorf("parent .xinwiki not created: %v", err)
	}
