...
// $XDG_CONFIG_HOME/xinwiki/secrets/, used as a fallback when no keyring
// backend is available).
//
// Namespace convention: "xinwiki:<profile>:<key>" where key is "access",
// "refresh", or "api_key".
package secrets
...
import (
	...
	"github.com/Tencent/XinWiki/cli/internal/xdg"
)
...
// keychain://xinwiki/<profile>/<key>) describing where a saved secret lives.
...
// FileStore writes 0600 plain-text files under $XDG_CONFIG_HOME/xinwiki/secrets/<profile>.
// It is the headless / CI default and the keychain fallback.
type FileStore struct {
	root string
}

// NewFileStore returns a FileStore rooted at $XDG_CONFIG_HOME/xinwiki/secrets
// (or ~/.config/xinwiki/secrets if XDG_CONFIG_HOME is unset). Same convention
