// Package skillparity contains the K6 drift guard: every xinwiki command and
...
func TestSkillsReferenceLiveCommandsAndFlags(t *testing.T) {
	...
	for _, path := range files {
		...
		for _, block := range codeFence.FindAllStringSubmatch(content, -1) {
			for _, line := range strings.Split(block[1], "\n") {
				idx := strings.Index(line, "xinwiki ")
				if idx < 0 {
					continue
				}
				inv := line[idx+len("xinwiki"):]
				...
				// Placeholder command (e.g. `xinwiki <command> --help`) → skip.
	...
	if !checkedAny {
		t.Fatal("parser resolved no xinwiki commands from skills — check tokenizer")
	}
}
