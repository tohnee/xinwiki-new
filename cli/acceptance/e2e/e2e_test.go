...
// Package e2e_test drives the XinWiki CLI binary against a real running
...
// CLI finds a profile without needing `xinwiki profile add` (which prompts
...
func writeSampleDoc(t *testing.T) string {
	t.Helper()
	content := `XinWiki E2E Sample Document

This sample document is used by the XinWiki CLI acceptance test suite to
validate the end-to-end retrieval-augmented generation pipeline.

向量检索的核心思想是把文本通过 embedding 模型映射到高维向量空间,然后通过余弦相似度
等度量找出语义最接近的内容片段。XinWiki 支持 vector + keyword 的混合检索模式。
...
// indexing is complete. XinWiki server uses a few status values across versions
